...
public class GridLayout implements ILayout {
    ...
    public void updateBoundsForChildren(IPencil pen, IBounds parent, List<WaveComponent> children, List<Object> childArgs, List<IEditableBounds> bounds) {
        ...
        for(int i=0;i<children.size();i++){
            if(!children.get(i).visible().get()) numChildren--;
        }
        ...
        for(int y=0;y<gridHeight;y++){
            ...
            for(int x=0;x<gridWidth;x++){
                ...
                if(id != null) {
                    ...
                    if(childBounds == null){
                        bounds.set(id, new ValueBounds(curX, curY, cellWidth, cellHeight));
                    }
                    else {
    ...
    private Float[] calculateWidthsForRows(IPencil pen, List<WaveComponent> children, List<Object> childArgs, int gridWidth, int gridHeight, float parentWidth){
    ...
    private Float[] calculateHeightsForCols(IPencil pen, List<WaveComponent> children, List<Object> childArgs, int gridWidth, int gridHeight, float parentHeight){
    ...
    private float maxMinCellHeightForColumn(IPencil pen, List<WaveComponent> children, List<Object> childArgs, int col, int gridWidth, int gridHeight){
    ...
    private float maxMinCellWidthForRow(IPencil pen, List<WaveComponent> children, List<Object> childArgs, int row, int gridWidth, int gridHeight){
        float maxMinWidth = 0;
